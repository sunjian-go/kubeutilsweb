      ...
      <el-aside :width="asideWidth" class="aside" :style="dynamicStyle">
        ...
        >
          ...
          >
            ...
            <!--index表示点击该路由时跳转到的页面-->
            <!-- && m.children[0].meta['role']=='admin' -->
            <div>
              <el-menu-item
                class="aside-menu-item"
                style="height: 50px"
                v-if="m.children && m.children.length == 1&&authUser(m.children[0].path)"
  ...
  watch: {
    $route: {
      handler(newRoute) {
        ...
        if (newPath == "/home" && sessionStorage.getItem("flag") != 1) {
          //sessionStorage.getItem('flag')!=1,表示只有第一次跳转该页面的时候flag才会不等于1，只要跳转过一次，那么后面都会等于1
          this.defaultActive = "";
          sessionStorage.setItem("flag", 1);
          // sessionStorage.setItem('path', '/home');
        } else if (
          this.defaultActive !== newPath &&
          newPath != "/workload/create"
        ) {
          // $route.path 发生变化
          this.defaultActive = newPath;
...
::v-deep .el-sub-menu__title:hover {
  /*::v-deep穿透，穿透到el-sub-menu的样式里面修改*/
